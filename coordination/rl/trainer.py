...
def get_subdiv_space(env, subdiv):
    ...
    # Output an array of tuple, [( [ant_1, box_1], [ant_1] ), (...)]    
    if subdiv:
...
class Trainer(object):
    ...
    def __init__(self, config):
        ...
        # for each primitive skill, actor network is a 3-layer MLP with hidden layer size 64, last layer is two end-headed, one for mean and one for std
        # at this point, actor and critic are not initialized yet. Note that, DIAYN is in following and will be initialized when actor is initialized
        ...
        if config.meta:
            ...
            if config.diayn:
                for cluster in clusters:
                    ...
                    # add entry like  ant_1,box_1_diayn
